    ...
    <section class="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 shadow-lg shadow-slate-950/40">
        <div class="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            ...
            <details class="relative">
                <summary class="flex cursor-pointer list-none items-center justify-center rounded-2xl border border-slate-700 bg-slate-800/70 px-3 py-2 text-sm font-semibold text-slate-200 transition hover:bg-slate-800">
                    ⋮
                </summary>
                <div class="absolute right-0 mt-2 w-40 rounded-2xl border border-slate-700 bg-slate-950 p-2 text-sm text-slate-200 shadow-lg">
                    <button class="w-full rounded-xl px-3 py-2 text-left hover:bg-slate-800">Exporter PDF</button>
                    <button class="mt-1 w-full rounded-xl px-3 py-2 text-left hover:bg-slate-800">Exporter CSV</button>
                </div>
            </details>
        </div>
    </section>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <div class="mt-2 grid gap-2 sm:grid-cols-2">
                    <input wire:model="periodeDebut" type="month" class="w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100" />
                    <input wire:model="periodeFin" type="month" class="w-full rounded-2xl border border-slate-800 bg-slate-950 px-3 py-2 text-sm text-slate-100" />
                </div>
            </div>
            <div>
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            @forelse ($this->factures as $facture)
                ...
                <div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-4">
                    ...
                    <div class="mt-4 grid gap-3 text-xs text-slate-400 sm:grid-cols-2">
                        <div>
                            <p>Net à payer</p>
                    ...
                    <button
                        ...
                        class="mt-4 w-full rounded-2xl border border-emerald-500/40 bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-100 transition hover:bg-emerald-500/30"
                    >
                        Voir détail
                    </button>
                    @if ($isActive)
                        <p class="mt-2 text-xs text-emerald-300">Facture sélectionnée</p>
                    @endif
                </div>
            @empty
        ...
        @if ($this->factureSelectionnee)
            ...
            <div class="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <span class="rounded-full border border-slate-700 bg-slate-950 px-3 py-2 text-xs font-semibold text-slate-200">
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                    ...
                    <div class="rounded-2xl border border-slate-800 bg-slate-950 px-4 py-3">
                        ...
                        <p class="mt-1 text-lg font-semibold text-emerald-200">{{ number_format($facture['total_verse'], 0, ',', ' ') }} FCFA</p>
                    </div>
                    <div class="rounded-2xl border border-slate-800 bg-slate-950 px-4 py-3">
                ...
                <div class="mt-6 space-y-3">
                    <details class="rounded-2xl border border-slate-800 bg-slate-950 px-4 py-3">
                        <summary class="cursor-pointer list-none text-sm font-semibold text-white">Versements</summary>
                        <div class="mt-4 space-y-4 text-sm text-slate-200">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid gap-3 sm:grid-cols-2">
                                    <input wire:model="versementForm.montant" type="number" min="0" placeholder="Montant" class="rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100" />
                                    <input wire:model="versementForm.date" type="date" class="rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100" />
                                </div>
                                <select wire:model="versementForm.mode" class="w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100">
                                    <option value="especes">Espèces</option>
                                    <option value="mobile_money">Mobile money</option>
                                    <option value="virement">Virement</option>
                                    <option value="carte">Carte</option>
                                </select>
                                <details class="rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2">
                    ...
                    <details class="rounded-2xl border border-slate-800 bg-slate-950 px-4 py-3">
                        <summary class="cursor-pointer list-none text-sm font-semibold text-white">Remises</summary>
                        <div class="mt-4 space-y-4 text-sm text-slate-200">
                            <div class="space-y-3">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid gap-3 sm:grid-cols-2">
                                    <select wire:model="remiseForm.type" class="rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100">
                                        <option value="pourcentage">Pourcentage</option>
                                        <option value="montant">Montant fixe</option>
                                    </select>
                                    <input wire:model="remiseForm.valeur" type="number" min="0" placeholder="Valeur" class="rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100" />
                                </div>
                                <input wire:model="remiseForm.commentaire" type="text" placeholder="Commentaire (optionnel)" class="w-full rounded-2xl border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100" />
                    ...
                    <details class="rounded-2xl border border-slate-800 bg-slate-950 px-4 py-3">
                        <summary class="cursor-pointer list-none text-sm font-semibold text-white">Historique</summary>
                        <div class="mt-4 space-y-3 text-sm text-slate-200">
                            @if ($eleveSelectionne)
                                @foreach ($eleveSelectionne['factures'] as $factureEleve)
                                    <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-3">
                                        <p class="font-semibold text-white">{{ $factureEleve['periode'] }}</p>
                                ...
                                @endforeach
                            @else
                                <p class="text-slate-400">Historique indisponible.</p>
                            @endif
                        </div>
                    </details>
                </div>
