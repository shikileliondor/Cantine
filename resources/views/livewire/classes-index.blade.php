<div class="space-y-8">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-white">Classes</h1>
            <p class="mt-1 text-sm text-slate-400">Structure scolaire et effectifs en temps réel.</p>
        </div>
    ...
    <div class="grid gap-4 lg:grid-cols-3">
        @foreach ($classes as $classe)
            <div wire:key="classe-card-{{ $classe->id }}" class="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-xs uppercase tracking-[0.2em] text-slate-500">{{ $classe->niveau ?? 'Niveau' }}</p>
                        <h2 class="text-xl font-semibold text-white">{{ $classe->nom }}</h2>
                    </div>
                    <x-cantine.badge tone="violet">{{ $classe->eleves_count }} élèves</x-cantine.badge>
                </div>
                <p class="mt-4 text-sm text-slate-400">Année scolaire : {{ $classe->annee_scolaire ?? 'Non définie' }}</p>
                <div class="mt-6 flex flex-wrap gap-2">
                    <a href="{{ route('eleves.classes.show', $classe) }}" class="rounded-xl border border-slate-800 px-3 py-2 text-xs text-slate-300 transition hover:border-emerald-400 hover:text-emerald-300">
                        Voir élèves
                    </a>
                    <button type="button" wire:click="openForm({{ $classe->id }})" class="rounded-xl border border-slate-800 px-3 py-2 text-xs text-slate-300 transition hover:border-emerald-400 hover:text-emerald-300">
                        Modifier
                    </button>
                    <button
                        type="button"
                        ...
                        class="rounded-xl border border-slate-800 px-3 py-2 text-xs text-slate-300 transition hover:border-rose-400 hover:text-rose-300"
                    >
                        Supprimer
                    </button>
                </div>
            ...
            <div class="w-full max-w-xl rounded-3xl border border-slate-800 bg-slate-900 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-xl font-semibold text-white">{{ $editingId ? 'Modifier la classe' : 'Nouvelle classe' }}</h3>
                    </div>
                    <button type="button" wire:click="closeForm" class="rounded-2xl border border-slate-800 px-4 py-2 text-sm text-slate-300">
                        Fermer
                    </button>
                </div>
                ...
                <div class="mt-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="niveau" class="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm text-slate-200 focus:border-emerald-400 focus:outline-none" />
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="annee_scolaire" placeholder="2024-2025" class="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-sm text-slate-200 focus:border-emerald-400 focus:outline-none" />
