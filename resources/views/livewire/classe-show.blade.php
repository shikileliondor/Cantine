<div class="space-y-8">
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-white">{{ $classe->nom }}</h1>
            <p class="mt-1 text-sm text-slate-400">Niveau : {{ $classe->niveau ?? 'Non défini' }} · Année scolaire : {{ $classe->annee_scolaire ?? 'Non définie' }}</p>
        </div>
    ...
    <div class="grid gap-4 md:grid-cols-2">
        <div class="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
            ...
            <p class="mt-3 text-3xl font-semibold text-white">{{ $classe->eleves->count() }}</p>
        </div>
        <div class="rounded-3xl border border-slate-800 bg-slate-900/60 p-6">
            <p class="text-xs uppercase tracking-[0.2em] text-slate-500">Statut cantine</p>
            <p class="mt-3 text-3xl font-semibold text-white">{{ $this->stats['payes'] }} payés / {{ $this->stats['impayes'] }} en attente</p>
            <p class="mt-2 text-sm text-slate-400">Basé sur la dernière facture connue.</p>
        </div>
    </div>
    ...
    <div class="rounded-3xl border border-slate-800 bg-slate-900/60">
        ...
        <div class="divide-y divide-slate-800">
            ...
            @empty
                <div class="px-6 py-6 text-sm text-slate-400">Aucun élève dans cette classe.</div>
            @endforelse
        </div>
    </div>
</div>
